...
#    union = Decimal(len(set.union(a,b,c,d)))
    ...
    # get sizes for regions delimited by set areas
    # _ is for - as in substract, i is for intersection, u is for union
    return { 'A_BuCuD': str(len(a - set.union(b,c,d))),
             'B_AuCuD': str(len(b - set.union(a,c,d))),
             'C_AuBuD': str(len(c - set.union(a,b,d))),
             'D_AuBuC': str(len(d - set.union(a,b,c))),

             'AiB_CuD': str(len(set.intersection(a,b) - set.union(c,d))),
             'AiC_BuD': str(len(set.intersection(a,c) - set.union(b,d))),
             'AiD_BuC': str(len(set.intersection(a,d) - set.union(b,c))),
             'BiC_AuD': str(len(set.intersection(b,c) - set.union(a,d))),
             'BiD_AuC': str(len(set.intersection(b,d) - set.union(a,c))),
             'CiD_AuB': str(len(set.intersection(c,d) - set.union(a,b))),

             'AiBiC_D': str(len(set.intersection(a,b,c) - d)),
             'BiCiD_A': str(len(set.intersection(b,c,d) - a)),
             'AiCiD_B': str(len(set.intersection(a,c,d) - b)),
             'AiBiD_C': str(len(set.intersection(a,b,d) - c)),

             'AiBiCiD': str(len(set.intersection(a,b,c,d))) }
...
def render_four_set_venn( a, b, c, d, titles={'A': 'A',
                                              ...
                                              'C': 'C',
                                              'D': 'D'} ):
    sections = fifteen_sections( a, b, c, d )
